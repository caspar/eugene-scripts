...
def pascal(n):
    """
    Yield up to row ``n`` of Pascal's triangle, one row at a time. The first row is row 0.
    """
...
def pascal_latex(n, out=sys.stdout):
    """
    Generate a Pascal triangle for LaTeX embedding. Sends output to the file-like object ``out`` (default: sys.stdout).
    """
